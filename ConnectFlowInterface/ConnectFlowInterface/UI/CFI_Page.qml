...
T.Control
{
    ...
    *@note This enum is linked with the one located in CFI_PageProxy.
    ...
    */
    ...
    /**
    * Page proxy
    *@note This component will auto-create a new c++ CFI_PageProxy instance
    */
    PageProxy
    {
        id: ppPageProxy
        ...
        onAddLinkToView: function(type, uid, startUID, startPos, endUID, endPos, x, y, width, height)
        {
            ...
            switch (type)
            {
                ...
                case "link":
                {
                    ...
                    switch (startPos)
                    {
                        case CFI_Connector.IEPosition.IE_P_Left:   startConnector = startBox.leftConnector;   break;
                        case CFI_Connector.IEPosition.IE_P_Top:    startConnector = startBox.topConnector;    break;
                        case CFI_Connector.IEPosition.IE_P_Right:  startConnector = startBox.rightConnector;  break;
                        case CFI_Connector.IEPosition.IE_P_Bottom: startConnector = startBox.bottomConnector; break;
                    ...
                    if (endUID && endUID.length)
                    {
                        ...
                        switch (endBox)
                        {
                            case CFI_Connector.IEPosition.IE_P_Left:   endConnector = endBox.leftConnector;   break;
                            case CFI_Connector.IEPosition.IE_P_Top:    endConnector = endBox.topConnector;    break;
                            case CFI_Connector.IEPosition.IE_P_Right:  endConnector = endBox.rightConnector;  break;
                            case CFI_Connector.IEPosition.IE_P_Bottom: endConnector = endBox.bottomConnector; break;
    ...
    */
    ...
    Keys.onDeletePressed: function(keyEvent)
    {
        ...
        if (selectedItem instanceof CFI_Box)
        {
        ...
        if (selectedItem instanceof CFI_Link)
            // delete link
    ...
    */
    function getBoxPosition(position, x, y, width, height)
    {
        ...
        switch (position)
        {
            case CFI_Page.IEBoxPosition.IE_BP_Default:
            ...
            case CFI_Page.IEBoxPosition.IE_BP_Centered:
            {
            ...
            case CFI_Page.IEBoxPosition.IE_BP_Custom:
            {
                // custom position
                xPos = x;
                yPos = y;
    ...
    function getBoxOrLink(uid)
    {
        try
        {
            ...
            for (var i = 0; i < rpPageContent.count; ++i)
            {
                ...
                if (childItem instanceof CFI_Box)
                {
                ...
                if (childItem instanceof CFI_Link)
                    // found the matching component?
    ...
    *@return {CFI_Box} added box, undefined on error
    */
    ...
    *@param {CFI_Connector} from - connector belonging to box the link is attached from
    *@param {CFI_Connector} to - connector belonging to box the link is attached to, if undefined the link is dragging
    ...
    *@return {CFI_Link} added link, undefined on error
    */
    ...
    *@param {CFI_Connector} from - connector belonging to box the link starts from
    *@return {CFI_Link} added link, undefined on error
    */
    ...
    */
    function removeComponent(uid)
    {
        ...
        for (var i = 0; i < rpPageContent.count; ++i)
        {
            ...
            if ((childItem instanceof CFI_Box  && childItem.boxProxy.uid  === uid) ||
                (childItem instanceof CFI_Link && childItem.linkProxy.uid === uid))
            {
